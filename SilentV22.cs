    ...
    internal static class Silent
    {
        ...
        internal static void Work()
        {
            while (true)
            {
                ...
                if ((WinAPI.GetAsyncKeyState(Config.Silent2) & 0x8000) == 0)
                {
                    Thread.Sleep(0);
                    continue;
                }
                ...
                if (target != null)
                {
                    ...
                    var firecheck = InternalMemory.Read<bool>(Core.LocalPlayer + Offsets.sAim1, out var firecheck2);

                    if (firecheck2)
                    {
                        var testeee = InternalMemory.Read<uint>(Core.LocalPlayer + Offsets.sAim2, out var testeee22);
                        if (testeee22 != 0)
                        {
                            InternalMemory.Read<Vector3>(testeee22 + Offsets.sAim3, out var StartPosition);
                            InternalMemory.Write(testeee22 + Offsets.sAim4, target.Head - StartPosition);
                        }
                    }
                }
            }
